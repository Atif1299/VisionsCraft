...
function createMinifiedVersions() {
  ...
  const css = fs.readFileSync(
    path.join(config.outputDir, 'combined.css'),
    'utf8'
  )
  ...
  fs.writeFileSync(
    path.join(config.outputDir, 'combined.min.js'),
    safeMinifiedJS
  )
...
function generateReport() {
  ...
  console.log(
    'Update your HTML to use combined.min.css and combined.min.js for production'
  )
}
